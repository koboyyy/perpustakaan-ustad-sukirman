...
<div id="navbar"
  class="bg-[#212A3E] text-[#EEF3F7] fixed left-0 top-0 w-full z-9999 font-sans shadow-lg">
  {{-- <div class="w-full h-1 bg-linear-to-r from-[#212A3E] via-[#394867] to-[#9BA4B5]"></div> --}}

  <nav class="relative px-3 md:px-10 flex items-center h-[60px] select-none">
    <!-- Left: Logo -->
    <a href="{{ route('home') }}" class="flex items-center gap-3 min-w-fit z-20">
      ...
      <div class="leading-tight">
        ...
        <span class="block font-medium text-[12px] text-[rgb(251,251,251)]">
          USTADZ SUKIRMAN <span class="text-[rgb(251,251,251,0.2)]">DESA WONOSARI</span>
        </span>
      </div>
    </a>
    ...
    <div class="flex items-center gap-2 md:gap-4 ml-auto z-20">
      ...
      @guest
        <a id="btn-login" href="{{ route('login') }}"
          class="px-4 py-2 rounded-md bg-[rgb(255,109,31)] text-[#F1F6F9] font-semibold shadow hover:bg-[#3170ad] transition flex items-center gap-2">
      ...
      @auth
        <form method="POST" action="{{ route('logout') }}" class="lg:inline-block m-0 p-0 hidden">
          @csrf
          <button type="submit"
            class="px-4 py-2 rounded-md bg-[#394867] text-[#F1F6F9] font-semibold shadow hover:bg-[#21324b] transition flex items-center gap-2 hover:cursor-pointer">
      ...
      <button id="hamburger-btn"
        class="lg:hidden ml-1 flex items-center justify-center p-2 text-[rgb(255,109,31)] hover:bg-[#E0E8EF] rounded transition"
    ...
    <div id="mobile-menu"
      ...
      <div
        ...
        <button id="close-mobile-menu"
          class="rounded p-2 text-[rgb(255,109,31)] hover:bg-[#E0E8EF] transition"
      ...
      <div class="flex flex-col gap-2 px-8 pt-8 text-base font-medium">
        ...
        @can('admin')
          <a href="{{ route('analitik') }}"
            class="w-full mt-3 py-3 rounded-md flex items-center justify-center bg-[rgb(255,109,31)] text-white font-semibold hover:bg-[#3170ad] transition gap-2">
            <i class="fa-solid fa-gauge"></i>
            Dashboard Admin
          </a>
        @endcan
