<section class="col-span-7 z-100 sticky top-0 w-full">
  <div
      ...
      <div class="flex">
        <div class="flex items-center gap-[8px] sm:gap-[16px]">
          <img src="{{ asset('img/logo.png') }}" alt="logo"
            class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg shadow-md bg-linear-to-tr from-[#D9E4EC] to-[#B0C4D9] p-1">
    ...
    <button onclick="showMobileNavDashboard()" id="hamburger-btn"
      class="text-[#212A3E] flex md:hidden">
    ...
    {{-- mobileNav --}}
    <div id="mobileNav"
      class="fixed inset-0 top-0 left-0 w-full h-full z-50 bg-[#F1F6F9] p-6 flex flex-col gap-4 md:hidden transition-transform duration-300 translate-x-0"
      style="display: none;">
      {{-- Header --}}
      <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-3">
          <img src="{{ asset('img/logo.png') }}" alt="logo"
            class="w-8 h-8 rounded-lg shadow-md bg-linear-to-tr from-[#D9E4EC] to-[#B0C4D9] p-1">
          <span class="text-[15px] font-bold text-[#394867]">Dashboard Admin</span>
        </div>
      ...
      <form method="POST" action="{{ route('logout') }}" class="inline-block">
        @csrf
        <button type="submit"
          class="w-full px-4 py-2 rounded-lg bg-[rgb(25,25,25)] text-[#F1F6F9] font-semibold shadow-md hover:bg-[#212A3E] transition-colors duration-300 flex items-center gap-1">
          <i class="fa-solid fa-sign-out-alt mr-2"></i>
          <span class="xs:inline">Logout</span>
        </button>
      </form>
